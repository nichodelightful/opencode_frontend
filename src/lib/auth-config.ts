export function getAuthConfigurationError() {
  ...
  if (!process.env.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD.length < 8 || process.env.ADMIN_PASSWORD.length > 1024) return "ADMIN_PASSWORD must contain 8 to 1024 characters.";
